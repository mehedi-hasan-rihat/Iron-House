      ...
      <div className="overflow-x-auto border border-[#BFE01D]/15">
        <table className="w-full text-sm">
          ...
          stagger={0.035} y={12} blur={false}>
            ...
            {staffList.map((s) => (
              <tr key={s.id} className="bg-[#050505] hover:bg-[#0d0f08] transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-[#9aa87a]">{s.staffId}</td>
                <td className="px-4 py-3 text-[#f2f4e8] font-medium whitespace-nowrap">{s.name}</td>
                <td className="px-4 py-3 text-[#9aa87a]">{s.designation}</td>
                <td className="px-4 py-3">
                  <span className="text-[10px] uppercase tracking-[0.15em] px-2 py-0.5 rounded-sm text-[#BFE01D] bg-[#BFE01D]/10">
                    {s.user.role.name}
                  </span>
                </td>
                <td className="px-4 py-3 text-[#9aa87a]">{s.phone}</td>
