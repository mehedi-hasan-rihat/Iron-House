...
export default async function CustomerMembershipPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl text-white uppercase tracking-wide">Membership</h1>
        <p className="label text-[#bdbdbd] mt-1">{member.memberships.length} total memberships</p>
      </div>
        ...
        <div className="border p-6 space-y-4" style={{ borderColor: ACC }}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {[
              ...
              <div key={label}>
                <p className="label text-[#bdbdbd]">{label}</p>
      ...
      <div className="border border-[#1a1a1a] bg-[#0b0b0b] p-5">
        <h3 className="label text-[#bdbdbd] mb-4">Membership History</h3>
        {member.memberships.length === 0
          ? <p className="text-[#bdbdbd] text-xs">No memberships yet.</p>
          : (
            <div className="space-y-3">
              {member.memberships.map((ms) => (
                <div key={ms.id} className="flex items-start justify-between py-3 border-b border-[#1a1a1a] last:border-0">
                  <div>
                    <p className="text-white text-sm font-medium">{ms.plan.name}</p>
                    <p className="label text-[#bdbdbd] mt-0.5">
                      {new Date(ms.startDate).toLocaleDateString("en-BD")} – {new Date(ms.endDate).toLocaleDateString("en-BD")}
                    </p>
                    <p className="label text-[#bdbdbd]">৳{Number(ms.finalAmount).toLocaleString()}</p>
                  </div>
